...
add_action('plugins_loaded', 'init_hrs_particle_gateway');

function init_hrs_particle_gateway() {
...
class HighRiskShop_Instant_Payment_Gateway_Particle extends WC_Payment_Gateway {
    ...
    public function process_payment($order_id) {
        ...
        $hrs_particlenetwork_currency = get_woocommerce_currency();
...
if (is_wp_error($hrs_particlenetwork_gen_wallet)) {
    // Handle error
	...
	$hrs_particlenetwork_wallet_body = wp_remote_retrieve_body($hrs_particlenetwork_gen_wallet);
	$hrs_particlenetwork_wallet_decbody = json_decode($hrs_particlenetwork_wallet_body, true);
...
function hrs_particlenetwork_change_order_status_rest_endpoint() {
...
function hrs_particlenetwork_change_order_status_callback( $request ) {
    $order_id = absint($request->get_param( 'order_id' ));
	$hrs_particlenetworkgetnonce = sanitize_text_field($request->get_param( 'nonce' ));
	
	 // Verify nonce
    if ( empty( $hrs_particlenetworkgetnonce ) || ! wp_verify_nonce( $hrs_particlenetworkgetnonce, 'hrs_particlenetwork_nonce_' . $order_id ) ) {
